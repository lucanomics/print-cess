...
export class UpstashSessionStore implements SessionStore {
  ...
  public async get(sessionId: string): Promise<PrintSession | null> {
    const raw = await this.#redis.get<unknown>(sessionKey(sessionId));
  ...
  public async putReceipt(receipt: SessionReceipt): Promise<void> {
    ...
    await this.#redis.set(receiptKey(receipt.sessionId), JSON.stringify(receipt), { px: ttl });
  }

  public async getReceipt(sessionId: string): Promise<SessionReceipt | null> {
    const raw = await this.#redis.get<unknown>(receiptKey(sessionId));
  ...
  public async prepareCleanup(sessionId: string, now: number): Promise<CleanupPreparation> {
    const raw = await this.#redis.eval<[number, number, string], string>(
  ...
  public async listDueOrphans(now: number, limit: number): Promise<BlobOrphanRecord[]> {
    ...
    const sessionIds = await this.#redis.zrange<string[]>(ORPHAN_DUE_KEY, "-inf", now, {
      byScore: true,
      offset: 0,
      count: boundedLimit,
    });
    ...
    for (const sessionId of sessionIds) {
      const raw = await this.#redis.get<unknown>(orphanKey(sessionId));
      if (!raw) {
  ...
  private async mutate(
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      const result = await this.#redis.eval<[number, string, number], number>(
        CAS_SCRIPT,
